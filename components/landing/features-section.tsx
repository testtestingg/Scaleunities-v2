  ...
  return (
    <svg viewBox="0 0 200 160" className="w-full h-full">
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        const radius = 50;
        const x = parseFloat((100 + Math.cos(angle) * radius).toFixed(2));
        const y = parseFloat((80 + Math.sin(angle) * radius).toFixed(2));
        return (
          <g key={i}>
      ...
      <div className="max-w-350 mx-auto px-6 lg:px-12">
        <div className="mb-16 lg:mb-24">
          ...
          <h2 className={"text-4xl lg:text-6xl font-display tracking-tight transition-all duration-700 " + (isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
            Our Services. <br /> <span className="text-muted-foreground">Solutions built for growth.</span>
          </h2>
        </div>
        <div>
